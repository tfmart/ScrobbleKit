...
public extension SBKManager {
    ...
    func getCorrectedArtistName(_ artist: String) async throws -> String? {
    ...
    func getCorrectedArtistName(_ artist: String, _ completion: ((String?, Error?) -> Void)?) {
        let service = ArtistGetCorrectionService(artist: artist, apiKey: apiKey, secretKey: secret)
        service.start { response, error in
            ...
            guard let correctedName = response else {
