...
public extension SBKManager {
    /**
     ...
     - Parameters:
       ...
       - period: The time period over which to retrieve top albums for. Possible values: overall, 7day, 1month, 3month, 6month, 12month. Default value is nil.
     ...
     */
    func getTopAlbums(forUser user: String, period: SBKSearchPerdiod = .overall, limit: Int = 50, page: Int = 1) async throws -> SBKSearchResult<SBKAlbum> {
    ...
    /**
     ...
     - Note: See [Last.fm's user.getTopAlbums documentation](https://www.last.fm/api/show/user.getTopAlbums) for more information.
     */
    func getUserTopAlbums(forUser user: String, period: SBKSearchPerdiod = .overall, limit: Int = 50, page: Int = 1, completion: ((SBKSearchResult<SBKAlbum>?, Error?) -> Void)?) {
        let service = UserGetTopAlbumsService(
            user: user,
            period: period.rawValue,
            limit: limit,
